	...
	void update(int clock)
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {
				...
				plane->setEntDeptTime(clock - plane->service_time);
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				landing_queue->set_num_served();
				landing_queue->set_total_wait(clock - plane->arrival_time); ///wait time in landing queue is clock - arrival_time
				...
				*/
				my_random.next_int(max_service_time);
